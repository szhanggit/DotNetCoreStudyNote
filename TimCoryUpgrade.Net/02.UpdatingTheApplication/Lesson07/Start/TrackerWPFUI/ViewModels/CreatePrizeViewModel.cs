...
namespace TrackerWPFUI.ViewModels
{
    public class CreatePrizeViewModel : Screen
    {
        ...
        {
            await EventAggregationProvider.TrackerEventAggregor.PublishOnUIThreadAsync(new PrizeModel());
        ...
        {
            PrizeModel model = new PrizeModel { PlaceNumber = placeNumber,
                PlaceName = placeName, PrizeAmount = prizeAmount,
                PrizePercentage = prizePercentage };

            GlobalConfig.Connection.CreatePrize(model);

            // TODO - Close out the form and alert the calling form
            await EventAggregationProvider.TrackerEventAggregor.PublishOnUIThreadAsync(model);
            await this.TryCloseAsync();
        }
